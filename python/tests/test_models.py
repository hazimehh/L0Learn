...
def _sample_FitModel(
    loss: str = "SquaredError", intercept: bool = True, penalty: str = "L0L1"
):
    i = intercept
    beta1 = csc_matrix([[0, 0], [1, 2]])
    beta2 = csc_matrix([[0], [3]])
    beta3 = csc_matrix([[0, 0, 0], [4, 5, 6]])
...
def test_FitModel_coeff(sample_FitModel):
    np.testing.assert_array_equal(
        sample_FitModel.coeff().toarray(),
        np.array([[-1, -2, -3, -4, -5, -6], [0, 0, 0, 0, 0, 0], [1, 2, 3, 4, 5, 6]]),
    )
    np.testing.assert_array_equal(
        sample_FitModel.coeff(include_intercept=False).toarray(),
        np.array([[0, 0, 0, 0, 0, 0], [1, 2, 3, 4, 5, 6]]),
    )

    np.testing.assert_array_equal(
        sample_FitModel.coeff(gamma=1).toarray(),
        np.array(
            [
                [
                    -3,
    ...
    np.testing.assert_array_equal(
        sample_FitModel.coeff(gamma=1, include_intercept=False).toarray(),
        np.array(
            [
                [
                    0,
    ...
    np.testing.assert_array_equal(
        sample_FitModel.coeff(lambda_0=6).toarray(),
        np.array(
            [
                [
                    -2,
    ...
    np.testing.assert_array_equal(
        sample_FitModel.coeff(lambda_0=6, include_intercept=False).toarray(),
        np.array(
            [
                [
                    0,
    ...
    np.testing.assert_array_equal(
        sample_FitModel.coeff(gamma=0, lambda_0=6).toarray(),
        np.array(
            [
                [
                    -5,
    ...
    np.testing.assert_array_equal(
        sample_FitModel.coeff(gamma=0, lambda_0=6, include_intercept=False).toarray(),
        np.array(
            [
                [
                    0,
...
def test_characteristics(sample_FitModel):
    pd.testing.assert_frame_equal(
        sample_FitModel.characteristics(),
        pd.DataFrame(
            {
                "l0": [10, 5, 10, 10, 5, 0],
                "support_size": [0, 1, 3, 4, 5, 6],
    ...
    pd.testing.assert_frame_equal(
        sample_FitModel.characteristics(gamma=1),
        pd.DataFrame(
            {
                "l0": [10],
                "support_size": [3],
    ...
    pd.testing.assert_frame_equal(
        sample_FitModel.characteristics(lambda_0=6),
        pd.DataFrame(
            {
                "l0": [5],
                "support_size": [1],
    ...
    pd.testing.assert_frame_equal(
        sample_FitModel.characteristics(gamma=0, lambda_0=5),
        pd.DataFrame(
            {
                "l0": [5],
                "support_size": [5],
...
def test_regularization_loss(coeffs):
    ...
    if coeffs.shape[1] == 1:
        np.testing.assert_equal(
            (coeffs != 0).sum(), regularization_loss(coeffs_csc, l0=1)
        )
        np.testing.assert_equal(
            (coeffs != 0).sum() + sum(abs(coeffs)) + sum(coeffs**2),
        ...
        np.testing.assert_equal(
            (coeffs != 0).sum(axis=0),
            regularization_loss(coeffs_csc, l0=[1] * num_solutions),
        )
        np.testing.assert_equal(
            (coeffs != 0).sum(axis=0)
        ...
        np.testing.assert_equal(
            np.arange(num_solutions) * ((coeffs != 0).sum(axis=0))
            + abs(coeffs).sum(axis=0)
            + (coeffs**2).sum(axis=0),
...
def test_squared_error_testing_reg(y_y_hat_error, coeffs):
    ...
    np.testing.assert_array_almost_equal(
        squared_error(y, y_hat, coeffs_csc, l0=0, l1=0, l2=0), error
    )

    reg_error = (
        (coeffs != 0).sum(axis=0)
        + abs(coeffs).sum(axis=0)
        + np.square(coeffs).sum(axis=0)
    )
    np.testing.assert_array_almost_equal(
        squared_error(y, y_hat, coeffs_csc, l0=np.ones(10), l1=1, l2=1),
        error + reg_error,
    )
...
def test_logistic_loss_testing_reg(y_y_hat_error, coeffs):
    ...
    np.testing.assert_almost_equal(
        logistic_loss(y, y_hat, coeffs_csc, l0=0, l1=0, l2=0), error
    )
    ...
    np.testing.assert_array_almost_equal(
        logistic_loss(y, y_hat, coeffs_csc, l0=np.ones(10), l1=1, l2=1),
        error + reg_error,
    )
...
def test_squared_hinge_loss_reg(y_y_hat_error, coeffs):
    ...
    y, y_hat, error = y_y_hat_error
    np.testing.assert_almost_equal(
        squared_hinge_loss(y, y_hat, coeffs_csc, l0=0, l1=0, l2=0), error
    )
    ...
    np.testing.assert_array_almost_equal(
        squared_hinge_loss(y, y_hat, coeffs_csc, l0=np.ones(10), l1=1, l2=1),
        error + reg_error,
    )
...
def test_score(training, loss):
    ...
    scored = fit_model.score(
        x_training, y_training, lambda_0=10, gamma=2, training=training
    )

    if training:
        exra_args = {
            "coeffs": fit_model.coeff(lambda_0=10, gamma=2, include_intercept=False),
...
def test_predict(sample_FitModel):
    ...
    np.testing.assert_array_equal(
        sample_FitModel.predict(ones_1D, gamma=1), np.zeros([1, 1])
    )
    np.testing.assert_array_equal(
        sample_FitModel.predict(ones_2D, gamma=1), np.zeros([2, 1])
    )

    np.testing.assert_array_equal(
        sample_FitModel.predict(ones_1D, lambda_0=1), np.zeros([1, 1])
    )
    np.testing.assert_array_equal(
        sample_FitModel.predict(ones_2D, lambda_0=1), np.zeros([2, 1])
    )

    np.testing.assert_array_equal(
        sample_FitModel.predict(ones_1D, lambda_0=1, gamma=1), np.zeros([1, 1])
    )
    np.testing.assert_array_equal(
        sample_FitModel.predict(ones_2D, lambda_0=1, gamma=1), np.zeros([2, 1])
    )
    ...
    np.testing.assert_array_equal(
        sample_FitModel.predict(x_1D), np.arange(1, 7)[np.newaxis, :]
    )
    np.testing.assert_array_equal(
        sample_FitModel.predict(x_2D), np.tile(np.arange(1, 7), (2, 1))
    )

    np.testing.assert_array_equal(
        sample_FitModel.predict(x_1D, gamma=1), np.array([[3]])
    )
    np.testing.assert_array_equal(
        sample_FitModel.predict(x_2D, gamma=1), np.array([[3], [3]])
    )

    np.testing.assert_array_equal(
        sample_FitModel.predict(x_1D, lambda_0=1), np.array([[2]])
    )
    np.testing.assert_array_equal(
        sample_FitModel.predict(x_2D, lambda_0=1), np.array([[2], [2]])
    )

    np.testing.assert_array_equal(
        sample_FitModel.predict(x_1D, lambda_0=1, gamma=1), np.array([[3]])
    )
    np.testing.assert_array_equal(
        sample_FitModel.predict(x_2D, lambda_0=1, gamma=1), np.array([[3], [3]])
    )
