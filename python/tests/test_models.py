...
@fixture
def sample_CVFitModel(sample_FitModel: FitModel, num_folds: int=2):
    cvMeans = np.random.random(num_folds) + 2  # Ensure positive
    cvSTDs = np.random.random(num_folds)

    settings = sample_FitModel.settings
    settings['num_folds'] = num_folds

    return CVFitModel(settings=sample_FitModel.settings,
                      lambda_0=sample_FitModel.lambda_0,
                      gamma=sample_FitModel.gamma,
                      support_size=sample_FitModel.support_size,
                      coeffs=sample_FitModel.coeffs,
                      intercepts=sample_FitModel.intercepts,
