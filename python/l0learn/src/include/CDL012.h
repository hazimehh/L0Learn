...
template <class T>
inline void CDL012<T>::ApplyNewBi(const std::size_t i, const double Bi_old,
                                  const double Bi_new) {
...
template <class T>
inline void CDL012<T>::ApplyNewBiCWMinCheck(const std::size_t i,
                                            const double Bi_old,
                                            const double Bi_new) {
  this->r += matrix_column_mult(*(this->X), i, Bi_old - Bi_new);
  this->B[i] = Bi_new;
...
inline double CDL012<T>::Objective(const arma::vec &r, const beta_vector &B) {
  ...
  return 0.5 * arma::dot(r, r) + this->lambda0 * n_nonzero(this->B) +
...
inline double CDL012<T>::Objective() {
  ...
  return 0.5 * arma::dot(this->r, this->r) +
...
template <class T>
CDL012<T>::CDL012(const T &Xi, const arma::vec &yi, const Params<T> &P)
    : CD<T, CDL012<T>>(Xi, yi, P) {
