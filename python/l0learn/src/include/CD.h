...
class CDBase {
 protected:
  ...
  std::vector<std::size_t> OldOrder;  // Cycling order to be used after support
 ...
 public:
  ...
  std::size_t MaxIters;
  std::size_t
      CurrentIters;  // current number of iterations - maintained by Converged()
  const double rtol;
  const double atol;
  arma::vec Lows;
  arma::vec Highs;
...
class CD : public CDBase<T> {
 ...
 public:
  ...
  inline void ApplyNewBi(const std::size_t i, const double old_Bi,
                         const double new_Bi) {
    // Must be implemented in Child Classes
    throw NotImplemented();
  };
...
void CD<T, Derived>::UpdateBiWithBounds(const std::size_t i) {
  ...
  // The following functions are virtual and must be defined for any CD
  // implementation.
  //    GetBiValue
  //    GetBiValue
  //    GetBiReg
  ...
  const double grd_Bi =
      static_cast<Derived *>(this)->GetBiGrad(i);  // Gradient of Loss wrt to Bi

  (*this->Xtr)[i] =
      std::abs(grd_Bi);  // Store absolute value of gradient for later steps

  const double old_Bi =
      this->B[i];  // copy of old Bi to adjust residuals if Bi updates

  const double nrb_Bi =
      static_cast<Derived *>(this)->GetBiValue(old_Bi, grd_Bi);
  ...
  const double bnd_Bi =
      clamp(std::copysign(reg_Bi, nrb_Bi), this->Lows[i], this->Highs[i]);
  ...
  if (i < this->NoSelectK) {
    ...
    // If ideal non-bounded reg_Bi is less than threshold, coefficient is not
    // worth setting.
    if (old_Bi != 0) {
      static_cast<Derived *>(this)->ApplyNewBi(i, old_Bi, 0);
...
bool CD<T, Derived>::UpdateBiCWMinCheck(const std::size_t i, const bool Cwmin) {
  ...
  if (reg_Bi < this->thr + lambda1_fudge_factor) {
    ...
    // Rcpp::Rcout << "Old B[" << i << "] = " << old_Bi << ", New B[" << i << "]
    // = " << new_Bi << "\n";
...
template <class T, class Derived>
bool CD<T, Derived>::UpdateBiCWMinCheckWithBounds(const std::size_t i,
                                                  const bool Cwmin) {
  // See CD<T>::UpdateBi for documentation
  const double grd_Bi = static_cast<Derived *>(this)->GetBiGrad(i);
  const double old_Bi = 0;
...
CDBase<T>::CDBase(const T &Xi, const arma::vec &yi, const Params<T> &P)
    : lambda0{P.ModelParams[0]},
      lambda1{P.ModelParams[1]},
      lambda2{P.ModelParams[2]},
      intercept{P.intercept},
      withBounds{P.withBounds},
      y{yi},
      ModelParams{P.ModelParams},
      CyclingOrder{P.CyclingOrder},
      MaxIters{P.MaxIters},
      rtol{P.rtol},
      atol{P.atol},
      Lows{P.Lows},
...
FitResult<T> CDBase<T>::Fit() {
  // arma::cout << "CDBase<T>::Fit() Start\n";
  // std::this_thread::sleep_for(std::chrono::milliseconds(10));
  if (this->withBounds) {
    return this->_FitWithBounds();
  } else {
    // arma::cout << "CDBase<T>::Fit No Bounds() Start\n";
    // std::this_thread::sleep_for(std::chrono::milliseconds(10));
    return this->_Fit();
  }
}
...
bool CD<T, Derived>::isConverged() {
  ...
  // "\n"; Rcpp::Rcout << "Exit 1: " << (std::abs(objectiveold -
  // this->objective) <= this->rtol*objectiveold) << ", Exit 2: " <<
  // (this->objective <= 1e-12) << "\n";
  return std::abs(objectiveold - this->objective) <=
             this->rtol * objectiveold ||
...
bool CD<T, Derived>::CWMinCheckWithBounds() {
  ...
  set_difference(this->Range1p.begin(), this->Range1p.end(), S.begin(), S.end(),
                 back_inserter(Sc));
...
template <class T, class Derived>
CD<T, Derived>::CD(const T &Xi, const arma::vec &yi, const Params<T> &P)
    : CDBase<T>(Xi, yi, P) {
...
template <class T>
CDSwaps<T>::CDSwaps(const T &Xi, const arma::vec &yi, const Params<T> &Pi)
    : CDBase<T>(Xi, yi, Pi) {
