...
template <class T>
inline double CDL012SquaredHingeSwaps<T>::Objective(const arma::vec &onemyxb,
                                                    const beta_vector &B) {
...
inline double CDL012SquaredHingeSwaps<T>::Objective() {
  throw std::runtime_error(
      "CDL012SquaredHingeSwaps does not have this->onemyxb");
}

template <class T>
CDL012SquaredHingeSwaps<T>::CDL012SquaredHingeSwaps(const T &Xi,
                                                    const arma::vec &yi,
                                                    const Params<T> &Pi)
    : CDSwaps<T>(Xi, yi, Pi) {
  twolambda2 = 2 * this->lambda2;
  qp2lamda2 = (LipschitzConst +
               twolambda2);  // this is the univariate lipschitz
                            // constant of the differentiable objective
...
FitResult<T> CDL012SquaredHingeSwaps<T>::_Fit() {
  ...
  for (std::size_t t = 0; t < this->MaxNumSwaps; ++t) {
    // Rcpp::Rcout << "Swap Number: " << t << "|mean(onemyxb): " <<
    // arma::mean(onemyxb) << "\n";
    ...
    for (auto &j : NnzIndices) {
      arma::vec onemyxbnoj =
          onemyxb + this->B[j] * this->y % matrix_column_get(*(this->X), j);
      ...
      for (std::size_t i = 0; i < this->p; ++i) {
        if (this->B[i] == 0 && i >= this->NoSelectK) {
          ...
          double partial_i =
              arma::sum(2 * onemyxbnoj.elem(indices) %
                        (-(this->y.elem(indices) %
                           matrix_column_get(*(this->X), i).elem(indices))));
          ...
          if (std::abs(partial_i) >= this->lambda1 + stl0Lc) {
            ...
            while (!converged) {
              ...
              onemyxbnoji +=
                  (Biold - Binew) * this->y % matrix_column_get(*(this->X), i);

              arma::uvec indicesi = arma::find(onemyxbnoji > 0);
              partial_i =
                  arma::sum(2 * onemyxbnoji.elem(indicesi) %
                            (-this->y.elem(indicesi) %
                             matrix_column_get(*(this->X), i).elem(indicesi)));
