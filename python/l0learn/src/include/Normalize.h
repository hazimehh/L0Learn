...
template <typename T>
std::tuple<arma::vec, arma::vec, double, double> Normalize(
    const T &X, const arma::vec &y, T &X_normalized, arma::vec &y_normalized,
  ...
  if (Normalizey) {
    ...
    // Multiplying the learned Beta by BetaMultiplier gives the optimal Beta on
    // the original scale
  } else {
    y_normalized = y;
