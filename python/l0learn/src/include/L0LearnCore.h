...
struct cvfitmodel : fitmodel {
  ...
  cvfitmodel() = default;
  cvfitmodel(const cvfitmodel &) = default;
...
cvfitmodel L0LearnCV(
  ...
  for (std::size_t j = 0; j < nfolds; ++j) {
    ...
    // validationindicesarma contains the randomly permuted validation indices
    // as a uvec
    arma::uvec validationindicesarma;
    arma::uvec validationindicestemp =
        arma::conv_to<arma::uvec>::from(validationindices);
    ...
    arma::uvec trainingindicestemp =
        arma::conv_to<arma::uvec>::from(trainingindices);
