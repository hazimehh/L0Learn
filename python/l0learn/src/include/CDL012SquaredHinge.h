...
inline double CDL012SquaredHinge<T>::GetBiGrad(const std::size_t i) {
  // Rcpp::Rcout << "Grad stuff: " << arma::sum(2 * onemyxb.elem(indices) % (-
  // matrix_column_get(*Xy, i).elem(indices))  ) << "\n";
  return arma::sum(2 * onemyxb.elem(indices) %
                   (-matrix_column_get(*Xy, i).elem(indices))) +
         twolambda2 * this->B[i];
}

template <class T>
inline double CDL012SquaredHinge<T>::GetBiValue(const double old_Bi,
                                                const double grd_Bi) {
...
template <class T>
inline void CDL012SquaredHinge<T>::ApplyNewBi(const std::size_t i,
                                              const double Bi_old,
                                              const double Bi_new) {
...
template <class T>
inline void CDL012SquaredHinge<T>::ApplyNewBiCWMinCheck(const std::size_t i,
                                                        const double Bi_old,
                                                        const double Bi_new) {
  onemyxb += (Bi_old - Bi_new) * matrix_column_get(*(this->Xy), i);
  this->B[i] = Bi_new;
...
template <class T>
inline double CDL012SquaredHinge<T>::Objective(const arma::vec &onemyxb,
                                               const beta_vector &B) {
  auto l2norm = arma::norm(B, 2);
  arma::uvec indices = arma::find(onemyxb > 0);

  return arma::sum(onemyxb.elem(indices) % onemyxb.elem(indices)) +
         this->lambda0 * n_nonzero(B) + this->lambda1 * arma::norm(B, 1) +
         this->lambda2 * l2norm * l2norm;
}
...
inline double CDL012SquaredHinge<T>::Objective() {
  ...
  return arma::sum(onemyxb.elem(indices) % onemyxb.elem(indices)) +
         this->lambda0 * n_nonzero(this->B) +
         this->lambda1 * arma::norm(this->B, 1) +
         this->lambda2 * l2norm * l2norm;
}

template <class T>
CDL012SquaredHinge<T>::CDL012SquaredHinge(const T &Xi, const arma::vec &yi,
                                          const Params<T> &P)
    : CD<T, CDL012SquaredHinge<T>>(Xi, yi, P) {
...
FitResult<T> CDL012SquaredHinge<T>::_Fit() {
  ...
  for (auto t = 0; t < this->MaxIters; ++t) {
    ...
    if (this->intercept) {
      ...
      const double partial_b0 =
          arma::sum(2 * onemyxb.elem(indices) % -this->y.elem(indices));
      this->b0 -= partial_b0 /
