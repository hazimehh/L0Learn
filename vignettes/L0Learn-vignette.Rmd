...
### Coefficient Bounds
Often, due to domain knowledge a user might want to supply bounds for the value a coefficient might take. Starting in version 2.0.0, L0Learn supports bounds for CD algorithms for all losses and penalties. (Support for the CDPSI algorithim is a planned feature in later version.). By default, L0Learn does not apply bounds (or more accurately they are infinite $-∞ <= \beta_i <= ∞$). Users can supply global bounds by setting `lows` and `highs` to scalar values such that `lows <= 0`, `lows != highs`, and `highs >= 0`. In addition, both `lows` and `highs` can be a vector of length `p` which can allow for individual coefficients to have different bounds.
...
fit <- L0Learn.fit(X, y, penalty="L0L2", lambdaGrid=userLambda, nGamma = 3, maxSuppSize=1000)
```
...
L0Learn offers support for sparse matrices. Specifically, L0Learn supports the dgCMatrix class, the most common sparse matrix type. Unfortunately, other sparse types are not support and must be converted to a dgCMatrix to work with L0Learn. L0Learn keeps the matrix sparse internally and thus is highly memory efficient and can be extremely time efficient if the matrix is sufficiently sparse. 

There is no API change for sparse vs dense matrices. However, if `intercept=TRUE` (the default case), there will be slight differences in the intercept value due to standardization and normalization methods of sparse vs dense matrices. Otherwise, if `intercept=FALSE`, the results will be identical for both sparse and dense representations of the same data.

```{r}
X_sparse <- as(X, "dgCMatrix")
fit_dense <- L0Learn.fit(X, y, penalty="L0", intercept = FALSE)
fit_sparse <- L0Learn.fit(X_sparse, y, penalty="L0", intercept = FALSE)

all.equal(fit_dense, fit_sparse)

```
The intercept terms are slightly different for the different representations.
```{r}
X_sparse <- as(X, "dgCMatrix")
fit_dense <- L0Learn.fit(X, y, penalty="L0", intercept = TRUE)
fit_sparse <- L0Learn.fit(X_sparse, y, penalty="L0", intercept = TRUE)

print(fit_dense$a0[[1]][0:10])
print(fit_sparse$a0[[1]][0:10])
