...
template <>
std::vector< std::vector<std::unique_ptr<FitResult<arma::mat>> > > Grid2D<arma::mat>::Fit()
{
    ...
    for(unsigned int i=0; i<Lambdas2.size();++i) //auto &l : Lambdas2
    {
        *Xtr = Xtrvec;
        ...
        G.push_back(std::move(Grid1D<arma::mat>(*X, *y, PG).Fit()));
    }
...
template <>
std::vector< std::vector<std::unique_ptr<FitResult<arma::sp_mat>> > > Grid2D<arma::sp_mat>::Fit()
{
    ...
    if (PG.P.Specs.Logistic)
    {
        ...
        // T Xy =  X->each_col() % *y; operation is not defined on sp_mat.
        arma::sp_mat Xy = arma::sp_mat(*X);
    ...
    for(unsigned int i=0; i<Lambdas2.size();++i) //auto &l : Lambdas2
    {
        *Xtr = Xtrvec;
        ...
        //std::vector<std::unique_ptr<FitResult>> Gl();
        //auto Gl = Grid1D(*X, *y, PG).Fit();
        G.push_back(std::move(Grid1D<arma::sp_mat>(*X, *y, PG).Fit()));
    }
