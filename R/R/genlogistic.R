...
#' 1) Generate a data matrix, X, drawn from a multivariate Gaussian
#'    distribution with mean = 0, sigma = Sigma
#' 2) Generate a vector B with k entries set to 1 and the rest are zeros.
#' 3) Every coordinate yi of the outcome vector y exists in {0, 1}^n is sampled
...
#' @param rho The threshold for setting values to 0.  I
#' If |X(i, j)| > rho => X(i, j) <- 0
#' @param s Signal-to-noise parameter. As s -> +Inf, the data generated becomes
#'   linearly separable.
#' @param sigma Correlation matrix, defaults to I.
#' @param shuffle_B A boolean flag for whether or not to randomly shuffle the
#'  Beta vector, B.
...
#' X = data$X
#' y = data$y
GenSyntheticLogistic <- function(n,
                                 p,
                                 k,
                                 seed,
                                 rho=0,
    ...
    if (is.null(sigma)) {
        ...
        if ((ncol(sigma) != p) || (nrow(sigma) != p)) {
            stop("sigma must be a semi positive definite matrix of
                 side length p")
        }
