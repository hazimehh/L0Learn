...
#' 1) Generate a correlation matrix, SIG,  where item [i, j] = A^|i-j|.
#' 2) Draw from a Multivariate Normal Distribution using (mu and SIG)
#'    to generate X.
#' 3) Generate a vector B with every ~p/k entry set to 1 and the rest are zeros.
#' 4) Sample every element in the noise vector e from N(0,1). 4)
#'    Set y = XB + b0 + e.
...
#' @param mu The mean for drawing from the Multivariate Normal Distribution.
#' A scalar of vector of length p.
...
#' X = data$X
#' y = data$y
GenSyntheticHighCorr <- function(n,
                                 p,
                                 k,
                                 seed,
                                 rho=0,
