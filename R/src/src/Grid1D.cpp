...
template <class T> Grid1D<T>::~Grid1D() {
...
template <class T> std::vector<std::unique_ptr<FitResult<T>>> Grid1D<T>::Fit() {
  ...
  if (P.Specs.L0 || P.Specs.L0L2 || P.Specs.L0L1) {
    ...
    // Rcpp::Rcout << "P ModelParams: {" << P.ModelParams[0] << ", " <<
    // P.ModelParams[1] << ", " << P.ModelParams[2] << ", " << P.ModelParams[3]
    // <<   "}\n";
    ...
    // Lambdas = arma::logspace(std::log10(lambdamin), std::log10(lambdamax),
    // G_ncols); Lambdas = arma::flipud(Lambdas);
    ...
    for (std::size_t i = 0; i < G_ncols; ++i) {
      UserInterrupt();
      // Rcpp::Rcout << "Grid1D: " << i << "\n";
      FitResult<T> *prevresult =
          new FitResult<T>; // prevresult is ptr to the prev result object
      ...
      if (!prevskip) {
        ...
        if (PartialSort && p > 5000 + NoSelectK)
          std::partial_sort(idx.begin() + NoSelectK,
                            idx.begin() + 5000 + NoSelectK, idx.end(),
                            [this](std::size_t i1, std::size_t i2) {
                              return (*Xtr)[i1] > (*Xtr)[i2];
                            });
        else
          std::sort(idx.begin() + NoSelectK, idx.end(),
                    [this](std::size_t i1, std::size_t i2) {
                      return (*Xtr)[i1] > (*Xtr)[i2];
                    });
        P.CyclingOrder = 'u';
      ...
      if (i >= 1 && !scaledown && !LambdaU) {
        P.ModelParams[0] =
            (((Xrmax - P.ModelParams[1]) * (Xrmax - P.ModelParams[1])) /
             (2 * (Lipconst))) *
            0.99; // for numerical stability issues.

        if (P.ModelParams[0] >= prevresult->ModelParams[0]) {
          P.ModelParams[0] = prevresult->ModelParams[0] * 0.97;
        } // handles numerical instability.
      } else if (i >= 1 && !LambdaU) {
        P.ModelParams[0] = std::min(
            P.ModelParams[0] * ScaleDownFactor,
            (((Xrmax - P.ModelParams[1]) * (Xrmax - P.ModelParams[1])) /
             (2 * (Lipconst))) *
                0.97);
      ...
      if (!currentskip) {
        ...
        *result = Model->Fit();
        ...
        P.b0 = G.back()->b0;
        // Udate: After 1.1.0, P.r is automatically updated by the previous call
        // to CD
        //*P.r = G.back()->r;
      }
